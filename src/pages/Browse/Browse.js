...
const Browse = () => {
    ...
    useEffect(() => {
        const getMovies = async() => {
            const popular = await fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${process.env.REACT_APP_MOVIE_API_KEY}&language=en-US&page=1`);
            const popularDatas = await popular.json();
            setPopularMovies(popularDatas.results.slice(0,10));
            
            const top = await fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.REACT_APP_MOVIE_API_KEY}&language=en-US&page=1`);
            const topDatas = await top.json();
            setTopRatedMovies(topDatas.results.slice(0,10));
            
            const playing = await fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.REACT_APP_MOVIE_API_KEY}&language=en-US&page=1`);
            const playingDatas = await playing.json();
            setPlayingMovies(playingDatas.results.slice(0,10));
            
            const upcoming = await fetch(`https://api.themoviedb.org/3/movie/upcoming?api_key=${process.env.REACT_APP_MOVIE_API_KEY}&language=en-US&page=1`);
